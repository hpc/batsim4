...
int execute_parallel_task_sequentially(BatTask * btask,
                                        ...
                                        BatsimContext * context)
{
    ...
    xbt_assert(hosts_to_use.size()==1,"Error running task '%s' Currently only one host can be used to run exec tasks sequentially.  Hosts: %zu",
                                          task_name.c_str(),hosts_to_use.size());
    xbt_assert(computation_vector.size()==1,"Error running task '%s' Currently only 1 computation should be done since using only 1 core and 1 host. Computations: %zu",
                                            task_name.c_str(),computation_vector.size());
    XBT_INFO("about to execute sequentially");
    simgrid::s4u::ExecPtr ptask = simgrid::s4u::this_actor::exec_init(computation_vector[0]);
    //ptask->set_host(hosts_to_use[0]);
